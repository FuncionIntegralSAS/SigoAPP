...
- **Protección de Sesión Mock:** Si una petición enviada contiene un token de pruebas (`mock-token`), `AuthInterceptor.onError` descarta la llamada a `handleSessionExpired()`, evitando que un error 401 expulse al usuario durante pruebas locales.
...
- **Simulación Centralizada en Red (`MockHttpInterceptor`):** Para el "Entorno de Pruebas (MOCK)" iniciado en `AuthProvider.mockLogin()`, la instancia global de `Dio` integra `MockHttpInterceptor`. Al detectar `mock-token` en la cabecera `Authorization`, intercepta en `onRequest` y resuelve localmente con HTTP 200 y payloads coherentes (empresas, bodegas, artículos, traspasos, requisiciones y transacciones `code: 0`), sin enviar tráfico a la red y permitiendo navegar toda la app de forma autónoma.
